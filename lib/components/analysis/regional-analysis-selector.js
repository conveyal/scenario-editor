...
import {
  ...
  setRegionalAnalysisDisplayPercentile
...
export default function RegionalAnalysisSelector(p) {
  ...
  function onChange(a) {
    const _id = get(a, '_id')
    setActiveId(_id)
    setImmediate(() => dispatch(setActiveRegionalAnalysis(_id)))
  }
  ...
  function onChangePercentile(e) {
    dispatch(setRegionalAnalysisDisplayPercentile(e.target.value))
  }
  ...
  return (
    <>
      ...
      {activeId ? (
        <Stack className='DEV' spacing={4} mb={4}>
