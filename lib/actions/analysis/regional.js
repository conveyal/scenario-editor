...
export const setRegionalAnalysisDisplayCutoff = createAction(
  'set regional analysis display cutoff',
  function(cutoff) {
    setSearchParameter('cutoff', cutoff)
    return cutoff
  }
)
export const setRegionalAnalysisDisplayPercentile = createAction(
  'set regional analysis display percentile',
  function(p) {
    setSearchParameter('percentile', p)
    return p
  }
)
...
export const loadRegionalAnalysisGrid = _id => async (dispatch, getState) => {
  const {displayCutoff, displayPercentile, grids} = getState().regionalAnalyses

  if (
    !grids.find(
      g =>
        g.analysisId === _id &&
        g.cutoff === displayCutoff &&
        g.percentile === displayPercentile
    )
  ) {
    const rawGrid = await dispatch(
      fetchSignedS3Url(
        `${REGIONAL_URL}/${_id}/grid/grid?cutoff=${displayCutoff}&percentile=${displayPercentile}`
      )
    )
    ...
    grid.analysisId = _id
    grid.cutoff = displayCutoff
    grid.percentile = displayPercentile
    // Pass into the store
