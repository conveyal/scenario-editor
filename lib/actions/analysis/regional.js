...
const fiveMin = ms('5m')
const isTimePredictionValid = job =>
  job.complete > 0 &&
  (job.activeWorkers > 1 ||
    Date.now() - fiveMin > job.regionalAnalysis.createdAt)
function addStatusToJob(job) {
  const timeRemaining = predictJobTimeRemaining(
    job.jobId,
    job.complete,
    job.total,
    job.regionalAnalysis.createdAt
  )
...
export const loadActiveRegionalJobs = regionId =>
  fetch({
    ...
    next: res => setActiveRegionalJobs((res.value || []).map(addStatusToJob))
  })
