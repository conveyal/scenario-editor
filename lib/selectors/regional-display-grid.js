...
export default createSelector(
  ...
  selectDisplayCutoff,
  (grids, activeId, comparisonId, cutoff) => {
    const findId = id =>
      grids.find(g => g.analysisId === id && g.cutoff === cutoff)
    const activeGrid = findId(activeId)
    const comparisonGrid = findId(comparisonId)
