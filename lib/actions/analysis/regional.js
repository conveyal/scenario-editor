...
export const loadRegionalAnalysisGrid = _id => async (dispatch, getState) => {
  const {displayCutoff, grids} = getState().regionalAnalyses

  if (!grids.find(g => g.analysisId === _id && g.cutoff === displayCutoff)) {
    const rawGrid = await dispatch(
      fetchSignedS3Url(
        `${REGIONAL_URL}/${_id}/grid/grid?cutoff=${displayCutoff}`
      )
    )
    // Parse the headers and create a usable grid.
    const grid = createGrid(rawGrid)
    // Assign the regional analysis _id and cutoff
    grid.analysisId = _id
